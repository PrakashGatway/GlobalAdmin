...
const CourseForm = ({ course, onSubmit, onCancel, error, submitting }) => {
    ...
    useEffect(() => {
        if (course) {
            ...
            //                 description: course.requirementsDescription?.[key] || ''
            //             }
            //         });
            //     });
            // }
            
            const requirementsArray = Object.keys(course.requirements || {}).map(key => ({
                key,
                value: {
                    value: course.requirements[key]?.value || '',
                    description: course.requirements[key]?.description || ''
                }
            }));
    ...
    const addRequirement = () => {
        setFormData(prev => ({
            ...prev,
            requirements: [...prev.requirements, { key: '', value: '', description: '' }]
        }));
    };
    ...
    const handleSubmit = (e) => {
        ...
        const requirementsDescriptionObj = {};
        ...
        formData.requirements.forEach(req => {
            if (req.key && req.value) {
                requirementsObj[req.key] = {
                    ...
                    "des": req.value.description || req.description
                }
                // if (req.value.description || req.description) {
                //     requirementsObj[req.key] = req.value.description || req.description;
                // }
            }
        });
        ...
        const submitData = {
            ...
            requirements: requirementsObj || formData.requirements,
            docsRequired: docsArray,
    ...
    return (
        <CForm onSubmit={handleSubmit}>
                ...
                <CCol md={12}>
                    <CCard>
                        <CCardBody>
                                        ...
                                        <tbody>
                                            {formData.requirements.map((req, index) => (
                                                <tr key={index}>
                                                    ...
                                                    <td>
                                                        <CFormInput
                                                            placeholder="e.g., 60, 40, score in %, 18 years"
                                                            value={typeof req.value === 'object' ? req.value.value : req.value}
                                                            onChange={(e) => {
                                                                const val = e.target.value;
                                                                if (typeof req.value === 'object') {
                                                                    updateRequirement(index, 'value', { ...req.value, value: val });
                                                                } else {
                                                                    updateRequirement(index, 'value', val);
                                                                }
                                                            }}
                                                        />
                                                    </td>
                                                    <td>
                                                        <CFormInput
                                                            placeholder="Description (e.g., Minimum band score required)"
                                                            value={typeof req.value === 'object' ? req.value.description : req.description}
                                                            onChange={(e) => {
                                                                const desc = e.target.value;
                                                                if (typeof req.value === 'object') {
                                                                    updateRequirement(index, 'value', { ...req.value, description: desc });
                                                                } else {
                                                                    updateRequirement(index, 'description', desc);
                                                                }
                                                            }}
                                                        />
                                                    </td>
                            ...
                            <small className="text-muted text-secondary mt-2 d-block">
                                <strong>Note:</strong> Add eligibility requirements with their values and descriptions. 
                                Examples: IELTS with value "6.5" and description "Minimum band score required", 
                                GPA with value "3.0" and description "On a 4.0 scale"
                            </small>
                        </CCardBody>
                    </CCard>
                </CCol>
            ...
            {/* Documents Required Section - Commented out as in original */}
            {/* <CRow className="mb-3">
                <CCol md={12}>
                    <CCard>
                        <CCardBody>
            ...
            {/* Extra Content Type - Commented out as in original */}
